...
import (
	...
	"github.com/storozhukBM/thp/thp2"
	"github.com/storozhukBM/thp/thp3"
)

func BenchmarkChanThroughput(b *testing.B) {
	for pIdx := 1; pIdx < 32; pIdx *= 2 {
		for bufSize := 1; bufSize <= 1024; bufSize *= 2 {
			b.Run(fmt.Sprintf("type:%s;pCnt:%d;cCnt:%d;buf:%d", "thp3", pIdx, 8, bufSize), func(b *testing.B) {
				runThp3Chan(b, pIdx, 8, bufSize)
			})
		}
		for bufSize := 1; bufSize <= 1024; bufSize *= 2 {
			b.Run(fmt.Sprintf("type:%s;pCnt:%d;cCnt:%d;buf:%d", "thp2", pIdx, 8, bufSize), func(b *testing.B) {
				runThp2Chan(b, pIdx, 8, bufSize)
			})
		}
		for bufSize := 1; bufSize <= 1024; bufSize *= 2 {
...
//nolint:thelper // This is not exactly helper and in case of error we want to know line
func runThp2Chan(b *testing.B, producersCnt int, consumersCnt int, bufferSize int) {
	canRun := &sync.WaitGroup{}
	canRun.Add(1)

	ch, chCloser := thp2.NewChan[int](bufferSize)
...
//nolint:thelper // This is not exactly helper and in case of error we want to know line
func runThp3Chan(b *testing.B, producersCnt int, consumersCnt int, bufferSize int) {
	canRun := &sync.WaitGroup{}
	canRun.Add(1)

	ch, chCloser := thp3.NewChan[int](bufferSize)
